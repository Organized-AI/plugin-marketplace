...
const limit=50_000_000,chunkSize=48_000;
...
export function storagePlan(state,{backend='d1',skillId='my-skill'}={}){
 ...
 const manifest={format:'skill-loop-cloudflare-checkpoint',version:1,id,skillId,createdAt:new Date().toISOString(),encoding:'gzip-base64',bytes:compressed.length,rawBytes:raw.length,contentHash:digest(raw),chunks:chunks.map(({ordinal,sha256})=>({ordinal,sha256})),files:state.files.length,excluded:state.excluded,coverage:state.coverage};
 const namespace='skill-loop/v1/'+skillId+'/'+id;const checkpointKey=skillId+':'+id;
 const writes=backend==='d1'?chunks.map(c=>({sql:`INSERT OR IGNORE INTO skill_loop_checkpoint_chunks(checkpoint_id,ordinal,sha256,value) VALUES (${quote(checkpointKey)},${c.ordinal},${quote(c.sha256)},${quote(c.value)});`})):chunks.map(c=>({key:namespace+'/chunk-'+String(c.ordinal).padStart(6,'0'),value:c.value}));
...
export function verifyState(manifest,returnedChunks){
 if(manifest?.format!=='skill-loop-cloudflare-checkpoint'||manifest.version!==1||!Array.isArray(manifest.chunks)||manifest.chunks.length>3000||manifest.rawBytes>100_000_000||!Array.isArray(returnedChunks)||returnedChunks.length!==manifest.chunks.length)throw Error('Incomplete or invalid checkpoint manifest');
 const chunks=[...returnedChunks].sort((a,b)=>a.ordinal-b.ordinal);
 let encoded='';for(let i=0;i<chunks.length;i++){const c=chunks[i],m=manifest.chunks[i];if(c.ordinal!==i||m.ordinal!==i||typeof c.value!=='string'||c.value.length>chunkSize||digest(c.value)!==m.sha256)throw Error('Cloud storage readback mismatch');encoded+=c.value;}
