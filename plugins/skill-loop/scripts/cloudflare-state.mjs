import {sqlValue,readColumns,decodeChunk} from './cloudflare-codec.mjs';
...
export function storagePlan(state,{backend='d1',skillId='my-skill'}={}){
 ...
 const reads=backend==='d1'?chunks.map(c=>({sql:`SELECT ordinal,sha256,${readColumns(chunks[c.ordinal].value)} FROM skill_loop_checkpoint_chunks WHERE checkpoint_id=${quote(checkpointKey)} AND ordinal=${c.ordinal};`})):writes.map(x=>({key:x.key}));
 if(backend==='d1'&&[...writes,commit,...reads].some(q=>Buffer.byteLength(q.sql,'utf8')>4000))throw Error('D1 operation exceeds native-connector transfer budget; use a file-capable adapter.');
...
export function verifyState(manifest,returnedChunks){
 ...
 let encoded='';for(let i=0;i<chunks.length;i++){const c={...chunks[i],value:decodeChunk(chunks[i],Math.min(manifest.version===1?48000:chunkSize,Math.ceil(manifest.bytes/3)*4-encoded.length))},m=manifest.chunks[i];if(c.ordinal!==i||m.ordinal!==i||typeof c.value!=='string'||c.value.length>(manifest.version===1?48000:chunkSize)||digest(c.value)!==m.sha256)throw Error('Cloud storage readback mismatch');if(encoded.length+c.value.length>Math.ceil(manifest.bytes/3)*4)throw Error('Encoded checkpoint exceeds manifest size');encoded+=c.value;}
