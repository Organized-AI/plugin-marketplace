...
metadata:
  ...
  integrates_with:
    ...
    - verify-build (agent)
    - organized-codebase-applicator (skill) # Structure partner
    - long-runner (skill)
    - phased-build (skill)
...
|-------|---------|-------------|
| `organized-codebase-applicator` | Apply structure + Boris infrastructure | New/messy projects needing organization |
...
### With `organized-codebase-applicator`

**Boris + OCA = Complete Project Setup**

These two skills are complementary partners:
- **OCA** handles STRUCTURE (where files go, directory layout, cleanup)
- **Boris** handles WORKFLOW (verification, permissions, daily practices)

**When to invoke OCA from Boris:**

| Situation | Action |
|-----------|--------|
| Project has no `.claude/` directory | Invoke `organized-codebase-applicator` first |
| Project structure is messy/inconsistent | Invoke OCA Phases 1-6, then continue Boris |
| Need to create a plugin from project | Invoke OCA Phase 4 |
| Starting fresh on existing codebase | Run OCA, then Boris Session Start protocol |
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   1. STRUCTURE (organized-codebase-applicator)              │
│      ├── Phase 1: Analyze project                           │
│      ├── Phase 2: Cleanup old iterations                    │
│      ├── Phase 3: Create standard directories               │
│      ├── Phase 4: Plugin structure (optional)               │
│      ├── Phase 5: Migrate content                           │
│      ├── Phase 6: Finalize & commit                         │
│      └── Phase 7: Set up Boris infrastructure ──────────┐   │
│                                                         │   │
│   2. WORKFLOW (boris) ◀─────────────────────────────────┘   │
│      ├── Configure CLAUDE.md (use template)                 │
│      ├── Set up permissions (settings.json)                 │
│      ├── Install verification commands                      │
│      └── Begin daily workflow protocols                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**Quick Check - Does project need OCA first?**

```bash
# Check if organized structure exists
ls -la .claude/ PLANNING/ ARCHITECTURE/ 2>/dev/null

# If directories missing → Invoke organized-codebase-applicator
# If present → Proceed with Boris protocols
```

---

## 📋 Protocol 7: New/Unorganized Project

When encountering a project without Boris infrastructure:
...
1. CHECK STRUCTURE
   └── Does .claude/, PLANNING/, CLAUDE.md exist?

2. IF MISSING → INVOKE organized-codebase-applicator
   ├── Run Phases 1-6 (structure)
   └── Phase 7 creates Boris infrastructure

3. VERIFY INFRASTRUCTURE
   ├── .claude/settings.json exists?
   ├── .claude/commands/ has verify, commit, etc.?
   └── CLAUDE.md has proper sections?

4. BEGIN BORIS WORKFLOW
   └── Start with Protocol 1: Session Start
