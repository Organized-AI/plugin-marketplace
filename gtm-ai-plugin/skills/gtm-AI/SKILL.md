...
**Strategic container validation before publishing.** This hook MUST run before Phase 4 publish operations.
...
| Trigger | Type | Action |
|---------|------|--------|
| Phase 4 entry | Automatic | Full audit |
| Before `gtm_version publish` | Automatic | Full audit |
| After 5+ component changes | Automatic | Quick audit |
| `/gtm-audit` command | Manual | Full audit |
| `/gtm-status` command | Manual | Status only |
...
│  ✓ Tag-trigger-variable correlations    │
│  ✓ sGTM event_id deduplication          │
...
|-------|----------|----------|
| **Critical** | BLOCKS publish | Merge conflicts, missing vars, broken triggers |
| **Warning** | Prompts user | Naming violations, orphaned components |
| **Info** | Logged only | High tag count, no folder organization |

### Integration with Publish Flow

```
BEFORE Publishing:
1. Run pre-publish-audit hook
2. Review AUDIT-REPORT-{{timestamp}}.md
3. If CRITICAL → STOP and fix
4. If WARNING → Confirm with user
5. If CLEAN → Proceed to publish
6. Capture rollback version ID
```
...
```
Deploy LinkedIn tracking to GTM
→ Agent runs Phases 0-3
→ Phase 4: Pre-publish audit runs automatically
→ Agent shows summary: "0 critical, 2 warnings"
→ Prompts: "Proceed with publishing?"
→ Publishes if confirmed
```

**Manual (Command Mode):**
```
/gtm-audit --container both    # Full audit
/gtm-status --verbose          # Status check
/gtm-audit --fix               # Auto-fix issues
```

### Output

Creates `AUDIT-REPORT-{{timestamp}}.md` with:
- Summary table (critical/warning/info counts)
- Detailed issue listings with resolutions
- Rollback reference (previous live version)
